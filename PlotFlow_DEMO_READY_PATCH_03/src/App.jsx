...
function App() {
  ...
  const [sheetUrl, setSheetUrl] = useState(() => localStorage.getItem("plotflow-last-sheet-url-r1") || "");
  ...
  const [lotOverlays, setLotOverlays] = useState(() => loadLocalJson("plotflow-lot-overlays-r1", {}));
  ...
  useEffect(() => {
    ...
    async function loadBundledMasterplan() {
      try {
        const probe = await fetch(DEFAULT_MASTER_PDF_URL, { method: "HEAD", cache: "no-store" });
        const type = probe.headers.get("content-type") || "";
        if (!probe.ok || !type.toLowerCase().includes("pdf")) {
          if (!cancelled) setLocatorMessage("Chưa có masterplan mặc định. Có thể dùng Link hoặc Upload PDF.");
          return;
        }
        if (cancelled) return;
        setLocatorSourceMode("link");
        setLocatorUrl(DEFAULT_MASTER_PDF_URL);
        ...
        if (!cancelled) setLocatorMessage("Chưa auto-load được masterplan mặc định. Link/Upload vẫn dùng bình thường.");
      }
    }
    const timer = setTimeout(loadBundledMasterplan, 120);
  ...
  async function openLotEditor(code = selectedCode) {
    ...
    let imageSrc = floorplanImages[code];
    if (!imageSrc) {
      const clean = await renderPdfRegion(pdfDocRef.current, pageRender, view, { outputWidth: 1640, includeHighlight: false });
      imageSrc = clean.dataUrl;
    }
  ...
  function saveLotOverlay(overlay) {
    ...
    setLotOverlays(next);
    localStorage.setItem("plotflow-lot-overlays-r1", JSON.stringify(next));
  ...
  async function connectGoogleSheet(sourceUrl = sheetUrl) {
  ...
  async function renderResolvedUnits(nextResults, unitList = units) {
    ...
    for (const unit of unitList) {
      ...
      const view = { ...DEFAULT_FLOORPLAN_VIEW, ...(override?.view || {}) };
      const hq = await renderPdfRegion(pdfDocRef.current, pageRender, view, { outputWidth: 1084, includeHighlight: false });
  ...
  async function saveFineTune(view) {
    ...
    if (lotOverlays[code]) {
      ...
      localStorage.setItem("plotflow-lot-overlays-r1", JSON.stringify(nextLots));
    }
    const hq = await renderPdfRegion(pdfDocRef.current, fineTunePageRender, view, { outputWidth: 1084, includeHighlight: false });
  ...
  async function floorplanImagesForExport(unit, scale) {
    ...
    const vector = await renderPdfRegion(pdfDocRef.current, pageRender, view, {
      outputWidth: Math.max(1084, Math.round(542 * Math.max(1, scale))),
      includeHighlight: false,
      maxRenderScale: 128,
    });
