...
export default function PosterCanvas({
  ...
  function saveLayout() {
    localStorage.setItem("plotflow-layout-round1-v9", JSON.stringify(layout));
    localStorage.setItem("plotflow-grid-round1-v9", JSON.stringify(grid));
  }

  useEffect(() => {
    try {
      const savedLayout = localStorage.getItem("plotflow-layout-round1-v9");
      const savedGrid = localStorage.getItem("plotflow-grid-round1-v9");
      if (savedLayout) {
                ...
                {lotOverlay?.pin?.visible && assets.pin2D && (
                  <img
                    ...
                    style={{
                      ...
                      transform: `translate(-50%, -100%) rotate(${lotOverlay.pin.rotation || 0}deg) scale(${lotOverlay.pin.scale || 1})`,
                    }}
                  />
                )}
                {!isEditing && onEditLot && (
