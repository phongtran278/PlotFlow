...
export default function OverviewArrangeLogicRuntimeV2() {
  useEffect(() => {
    let disposed = false;
    let overlay = null;
    let stage = null;
    ...
    function renderSelected() {
      ...
      const autoGap = selectedSolution.gap < ui.gap ? ` · auto gap ${selectedSolution.gap}px` : ` · ${selectedSolution.gap}px gap`;
      if (footer) footer.textContent = `${items.length} cards${autoGap} · ${selectedSolution.lanes} lane${selectedSolution.lanes === 1 ? "" : "s"}${selectedSolution.ratio < 0.999 ? ` · auto-fit ${percent}%` : ""} · 0 connector conflicts`;
    ...
    function applySelected() {
      ...
      window.dispatchEvent(new CustomEvent("pf-overview-auto-arranged", { detail: { mode: selectedMode, count: items.length, gap: selectedSolution.gap, requestedGap: ui.gap, lanes: selectedSolution.lanes } }));
        ...
        <section class="pf-arrange-preview-panel" role="dialog" aria-modal="true" aria-label="Arrange preview">
          ...
          <div class="pf-arrange-preview-body">
            ...
            <aside class="pf-arrange-preview-modes">
              ...
              <button type="button" data-arrange-mode="right"><strong>All right</strong><small>Every card must stay on the right half</small></button>
            </aside>
          </div>
